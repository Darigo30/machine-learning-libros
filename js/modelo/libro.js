
const datosEntrenamientoLibros = [
    {
        input: {
            type: "Novela",
            autor: "Gabriel García Márquez",
            fecha: 1967,
            imagen: "https://images.cdn3.buscalibre.com/fit-in/360x360/61/8d/618d227e8967274cd9589a549adff52d.jpg"
        },
        output: {
            like: 0
        },
        details: {
            nombre: "Cien Años de Soledad",
            autor: "Gabriel García Márquez",
            imagen: "https://images.cdn3.buscalibre.com/fit-in/360x360/61/8d/618d227e8967274cd9589a549adff52d.jpg"
        }
    },
    {
        input: {
            type: "Fantasía",
            autor: "J.R.R. Tolkien",
            fecha: 1954,
            imagen: "https://images.cdn3.buscalibre.com/fit-in/360x360/54/49/5449ba87a3e457a22dd6d0972b5c261e.jpg"
        },
        output: {
            like: 1
        },
        details: {
            nombre: "El Señor de los Anillos",
            autor: "J.R.R. Tolkien",
            imagen: "https://images.cdn3.buscalibre.com/fit-in/360x360/54/49/5449ba87a3e457a22dd6d0972b5c261e.jpg"
        }
    },
    {
        input: {
            type: "Terror",
            autor: "William Peter Blatty",
            fecha: 1971,
            imagen: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1PWmssj_X-XzNJeuMNRok2eaQ_ZCu3ARmDFh-5p9t1Q&s"
        },
        output: {
            like: 0
        },
        details: {
            nombre: "El Exorcista",
            autor: "William Peter Blatty",
            imagen : "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR1PWmssj_X-XzNJeuMNRok2eaQ_ZCu3ARmDFh-5p9t1Q&s"
        }
    },
    {
        input: {
            type: "Fantasía",
            autor: "J.R.R. Tolkien",
            fecha: 1937,
            imagen: "https://http2.mlstatic.com/D_NQ_NP_891485-MLU70518134884_072023-O.webp"
        },
        output: {
            like: 1
        },
        details: {
            nombre: "El Hobbit",
            autor: "J.R.R. Tolkien",
            imagen: "https://http2.mlstatic.com/D_NQ_NP_891485-MLU70518134884_072023-O.webp"
        }
    },
    {
        input: {
            type: "Novela",
            autor: "Gabriel García Márquez",
            fecha: 1985,
            imagen: "https://images.cdn3.buscalibre.com/fit-in/360x360/6d/77/6d776dbe7bf4e8ad20b9107baa9ec852.jpg"
        },
        output: {
            like: 0
        },
        details: {
            nombre: "El Amor en los Tiempos del Cólera",
            autor: "Gabriel García Márquez",
            imagen: "https://images.cdn3.buscalibre.com/fit-in/360x360/6d/77/6d776dbe7bf4e8ad20b9107baa9ec852.jpg"
        }
    },
    {
        input: {
            type: "Terror",
            autor: "William Peter Blatty",
            fecha: 1974,
            imagen: "https://assets.lectulandia.com/b/ab/William%20Peter%20Blatty/Legion%20(1)/big.jpg"
        },
        output: {
            like: 0
        },
        details: {
            nombre: "Legión",
            autor: "William Peter Blatty",
            imagen: "https://assets.lectulandia.com/b/ab/William%20Peter%20Blatty/Legion%20(1)/big.jpg"
        }
    },
    {
        input: {
            type: "Fantasía",
            autor: "J.R.R. Tolkien",
            fecha: 1949,
            imagen: "https://images.cdn1.buscalibre.com/fit-in/360x360/35/2e/352e63af40b5cc4eada5cee03da49412.jpg"
        },
        output: {
            like: 0
        },
        details: {
            nombre: "El Señor de los Anillos: Las Dos Torres",
            autor: "J.R.R. Tolkien",
            imagen: "https://images.cdn1.buscalibre.com/fit-in/360x360/35/2e/352e63af40b5cc4eada5cee03da49412.jpg"
        }
    },
    {
        input: {
            type: "Novela",
            autor: "Gabriel García Márquez",
            fecha: 1981,
            imagen: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQs80PyWxw0gLOJPo0vnS7czcTAnLVthER82VrL9kO1AA&s"
        },
        output: {
            like: 1
        },
        details: {
            nombre: "Crónica de una Muerte Anunciada",
            autor: "Gabriel García Márquez"
        }
    },
    {
        input: {
            type: "Terror",
            autor: "William Peter Blatty",
            fecha: 1983,
            imagen: "https://wp.es.aleteia.org/wp-content/uploads/sites/7/2022/04/La-amarga-Pasion-de-Cristo-lavozdepapel.jpg?w=470"
        },
        output: {
            like: 1
        },
        details: {
            nombre: "Legión de Maria",
            autor: "Darita Gomez",
            imagen: "https://wp.es.aleteia.org/wp-content/uploads/sites/7/2022/04/La-amarga-Pasion-de-Cristo-lavozdepapel.jpg?w=470"
        }
    }
];




export default datosEntrenamientoLibros;